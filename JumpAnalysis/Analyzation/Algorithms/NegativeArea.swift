...
class NegativeArea: VisualizableAlgorithm, ParameterizedAlgorithmProtocol {
    var algorithmName = "Negative"
    ...
    func factory(parameters: [AlgorithmParameter]) -> AlgorithmProtocol {
        return NegativeArea(parameters: parameters)
    }
    ...
    override func calculateResult(sensorData: [SensorData]) -> (visualizationInformation: [VisualizationInformation], result: Double) {
        ...
        for data in sensorData {
            ...
            if wasNegativeAfterFirst && currentValue > 0 {
                lastPeak = data
                break
            }
